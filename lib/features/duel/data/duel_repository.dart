...
class DuelRepository {
  ...
  /// Salla Bakalım (Guess) sorularını çeker
  Future<List<DuelGuessQuestion>> getGuessQuestions({int count = 5}) async {
    try {
      final db = await _dbHelper.database;
      ...
      if (kDebugMode) {
        debugPrint('🔍 GuessLevels tablosundan ${levels.length} level bulundu');
      }
      ...
      final List<DuelGuessQuestion> allQuestions = [];
        ...
        if (questionsJson != null && questionsJson.isNotEmpty) {
          try {
            ...
            for (int i = 0; i < questions.length; i++) {
              ...
              allQuestions.add(
                DuelGuessQuestion(
                  id: '${level['guessID'] ?? level['levelID']}_$i',
